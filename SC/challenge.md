...
- In the Northwind database, what is the type of relationship between the
  ...
  In the Northwind database, the `Employee` and `Territory` tables are connected
  by the `EmployeeTerritory` table which describes which territories each employee
  works in. To connect information between `Employee` and `Territory` you just have
  to match up the appropriate IDs from each table involved. This means that 
  `Employee.EmployeeID = EmployeeTerritory.EmployeeID` and
  `EmployeeTerritory.TerritoryID = Territory.TerritoryID` Employees can work in 
  multiple territories which is a one-to-many relationship.
  
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  
  A document store like MongoDB is appropriate if you're looking to store large
  amounts of loosely structured data, perhaps data that's been automatically
  collected by user input. Document stores don't preserve data or the relationship
  between data well though. If you're looking to make complicated transactions on a
  relatively small amount of data, a relational database like SQLite or PostgreSQL 
  is more approprite. In the case that one of your complicated queries in a transaction 
  fails, it will roll back all changes made to the database, thus preserving the 
  original data.
  
- What is "NewSQL", and what is it trying to achieve?

  NewSQL is a class of databases that attempt to solve the scalabilty issues of
  SQL databases while preserving the data and relationships between data better.
  They still use SQL as their query language, and preserve the traits of Atomicity,
  Consistency, Isolation and Durability which are important to many business
  applications. These properties protect the veracity of the data, ensure that 
  concurrent transactions don't affect each other, and keep the data safe in the
